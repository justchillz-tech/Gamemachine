...
void addMole() {
  ...
  if (score >= Level500ms)  ttl = 500;

  int m;
  do {
    m = random(5);
  } while (moleActive[m] || digitalRead(buttonPins[m]) == LOW);

  moleActive[m] = true;
  digitalWrite(relayPins[m], LOW); // ON
  moleEnd[m] = millis() + ttl;
  molesLit++;

  Serial.print("MOLE ON → ");
  Serial.print(colors[m]);
  Serial.print(" (");
  Serial.print(ttl);
  Serial.println(" ms)");
}
